...
try {
    ...
    $stmt = $pdo->prepare("SELECT tracks.*, users.username FROM tracks JOIN users ON tracks.user_id = users.id WHERE users.role = 2 AND status = 1 ORDER BY tracks.created_at DESC"); 
    $stmt->execute();
